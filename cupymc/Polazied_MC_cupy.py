...
def main():
    ...
    with open('MonteCarlo/cupymc/config.yml', 'r') as file:
    ...
    p_vectors =parameters['photon']['p']
    d_vectors =parameters['photon']['d']
    ...
    MM = np.zeros((resolution_x,resolution_y,4,4,len(spectrum)))
    MM1 = np.zeros((resolution_x,resolution_y,4,4,len(spectrum)))
    D = np.zeros((len(spectrum),2))
    ...
    for idW,Lambda in  enumerate(spectrum):
        ...
        bs_camera = np.zeros((6, 4, resolution_x,resolution_y),dtype=np.complex_)
                ...
                for idx, event in enumerate(type_list):
                    ...
                    if event.startswith('sphere'):
                        if parameters['scattering'][event]['method'] == 'traditional':
                            ...
                            bs_camera[idm,:,:,:]= Scattering.Trans_cupy(resolution_x,resolution_y)
                            
                            # t_camera[idm,:,:,:],bs_camera[idm,:,:,:], depth_max, effective_number = Scattering.ParTrans(resolution_x,resolution_y,event)
                            # print(stokes_camera)
        ...
        # 5 plot the result
        s2m = mymath() 
        name = 'bs_'+str(Lambda)+'_'+str(N_photons)+'_'+str(pattern)+'_'+str(beam_radius)+'_'+str(r)+'_'+str(vf)+'_'+str(x_max)
        MM[:,:,:,:,idW]=s2m.sixstokes2mueller(bs_camera,resolution_x,resolution_y,polarized_light_type,name)
        
        # name1 = 't_'+str(Lambda)+'_'+str(N_photons)+'_'+str(pattern)+'_'+str(beam_radius)+'_'+str(r)+'_'+str(vf)+'_'+str(x_max)
    ...
    # for idW,Lambda in  enumerate(spectrum):
        ...
        # colorbar = plt.colorbar()
        # fig.savefig('result/v9/DP_'+str(Lambda)+'_'+str(N_photons)+'_'+str(pattern)+'_'+str(beam_radius)+'_'+str(r)+'_'+str(vf)+'_'+str(x_max)+'.png')
        # np.save('result/v9/DP_'+str(N_photons)+'_'+str(Lambda)+'_'+str(pattern)+'_'+str(beam_radius)+'_'+str(vf)+'_'+str(Npar)+'_'+str(event)+'_'+str(x_max)+'_'+str(D[idW,1]), MM[:,:,:,:,idW])
