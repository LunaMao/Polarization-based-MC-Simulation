...
class transmission(photon_event, position):
    ...
    def update_polarized_states(self,alfa,beta,p_vectors,d_vectors,stokes):
        ...
        # S=np.array([Io,Qo.item(),Uo.item(),Vo.item()],dtype=object)

        S=np.array([Io,Qo.item(),Uo.item(),Vo.item()])
        ...
        if sum(nan_probe)==0:
            flag_c=True
            D=np.array([ux_a.item(),uy_a.item(),uz_a.item()])
            return S,D,flag_c
        else:
    ...
    def Trans_cupy(self,resolution_x,resolution_y):
        ...
        for n in tqdm(range(1,int(self.number_photon)),ncols=80):

            stokes=self.stokes
            print("No.{} photon, launch ! !".format(n))
            # 5.7.3 initialize the position, polarized state, albedo and step
            # for each photons 
            p_vectors=P_vectors[n,:]
            d_vectors=D_vectors[n,:]
    ...
    def Trans(self,resolution_x,resolution_y,event):
    ...
    def ParTrans(self, resolution_x, resolution_y, event):
        ...
        with Pool(processes=2) as pool:  # Change the number of processes as needed
