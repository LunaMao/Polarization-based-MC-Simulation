...
class mymath:
    ...
    def sixstokes2mueller(self,S,sizex,sizey,polarized_light_type,name):
        ...
        M = torch.zeros(sizex,sizey,4,4).type(torch.cuda.FloatTensor)
        # m00
        M[:,:,0,0] = (LHP[0,:,:]+LVP[0,:,:])/2
        # m01 
        M[:,:,0,1] = (LHP[0,:,:]-LVP[0,:,:])/2
        ...
        fig.savefig('MonteCarlo/torchmc/result/v9/figure_ori'+str(name)+'.png')
        np.save('MonteCarlo/torchmc/result/v9/ori_'+str(name), M)
        ...
        #         # norm=np.amax(MM[:,:,row,colum,idW])
                ...
                colorbar = plt.colorbar()
                plt.clim(-1,1)
        fig.savefig('MonteCarlo/torchmc/result/v9/figure_nor1'+str(name)+'.png')
        np.save('MonteCarlo/torchmc/result/v9/nor1_'+str(name), M1)
        ...
        fig.savefig('MonteCarlo/torchmc/result/v9/figure_nor2'+str(name)+'.png')
        np.save('MonteCarlo/torchmc/result/v9/nor2_'+str(name), M2)
    ...
    def torch_two_dimensional_mask(self,p_vectors,d_vectors,albedo,stokes,alive_mask):

        # print("alive number is {}".format(alive_mask.sum()))
        m = alive_mask.squeeze()
        p_vectors = p_vectors[m,:]
        ...
        # d_vectors_x = d_vectors[:,0].unsqueeze(1)[alive_mask].reshape(-1,1)
        # d_vectors_y = d_vectors[:,1].unsqueeze(1)[alive_mask].reshape(-1,1)
        # d_vectors_z = d_vectors[:,2].unsqueeze(1)[alive_mask].reshape(-1,1)

        # d_vectors = torch.cat((d_vectors_x,d_vectors_y,d_vectors_z),dim=1)
        # # d_vectors = d_vectors[alive_mask.repeat(1,3)]
        
        # albedo = albedo[alive_mask].reshape(-1,1)

        # s0 = stokes[:,0].unsqueeze(1)[alive_mask].reshape(-1,1)
        # s1 = stokes[:,1].unsqueeze(1)[alive_mask].reshape(-1,1)
        # s2 = stokes[:,2].unsqueeze(1)[alive_mask].reshape(-1,1)
        # s3 = stokes[:,3].unsqueeze(1)[alive_mask].reshape(-1,1)

        # stokes = torch.cat((s0,s1,s2,s3),dim=1)

        return p_vectors, d_vectors, albedo, stokes
